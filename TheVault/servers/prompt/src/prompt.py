...
@app.prompt(output="q_ls,dkr_results,ersatz_chunks,template->prompt_ls")
def synthesis(
    q_ls: List[str],
    dkr_results: List[Dict[str, Any]],
    ersatz_chunks: List[List[Dict[str, Any]]],
    template: Union[str, Path]
) -> List[PromptMessage]:
    """Generate synthesis prompts for The Vault.

    Args:
        q_ls: List of user questions
        dkr_results: List of results from DKR (one per question)
        ersatz_chunks: List of chunk lists from Ersatz (one per question)
        template: Path to Jinja2 template file
    ...
    # Handle case where inputs might be incomplete or mismatched in length
    # UltraRAG generally ensures batch alignment, but safety first
    for i, q in enumerate(q_ls):
        docs = []
        
        # Add DKR Results
        if i < len(dkr_results):
            dkr_res = dkr_results[i]
            # dkr_res might be a dict with "results": [...] 
            # or the list itself depending on how the server returns it.
            # DKR server returns {"results": [...]} 
            if isinstance(dkr_res, dict) and "results" in dkr_res:
                for item in dkr_res["results"]:
                    docs.append(f"{item['content']}\n[Source: {item['source']}]")
        
        # Add Ersatz Results
        if i < len(ersatz_chunks):
            # Ersatz returns {"chunks": [...]} 
            ersatz_res = ersatz_chunks[i]
            # Based on how I defined Ersatz output in server:
            # output="answer,confidence,chunks"
            # UltraRAG will likely pass the 'chunks' part if I mapped it to ersatz_chunks
            # Wait, pipeline output mapping: chunks: ersatz_chunks
            # So ersatz_chunks here IS the list of chunks.
            if isinstance(ersatz_res, list):
                for item in ersatz_res:
                     docs.append(f"{item['content']}\n[Source: {item['source']} Confidence: {item.get('confidence', 0):.2f}]")
            elif isinstance(ersatz_res, dict) and "chunks" in ersatz_res:
                 # Just in case it passes the whole object
                 for item in ersatz_res["chunks"]:
                     docs.append(f"{item['content']}\n[Source: {item['source']} Confidence: {item.get('confidence', 0):.2f}]")

        doc_text = "\n\n".join(docs) if docs else "No relevant information found."
        
        p = template.render(question=q, documents=doc_text)
        ret.append(p)
